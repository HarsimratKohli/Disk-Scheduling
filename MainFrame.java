...
public class MainFrame extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("input")) {
			...
			prevposition=Integer.parseInt(JOptionPane.showInputDialog(this,
					"Enter prevposition: "));
			...
			if (selected == 0) {
				...
				ScanDisk scan = new ScanDisk(diskPosstion, startingPoint,prevposition);
				setGraphPanel(new DrawGraph(scan.calculateScanDisk()));
			} else if (selected == 3) {
				CScanDisk cscan = new CScanDisk(diskPosstion, startingPoint,prevposition);
				setGraphPanel(new DrawGraph(cscan.calculateScanDisk()));
			} else if (selected == 4) {
				LookDisk look = new LookDisk(diskPosstion, startingPoint,prevposition);
				setGraphPanel(new DrawGraph(look.calculate()));
			} else if (selected == 5) {
				ClookDisk clook = new ClookDisk(diskPosstion, startingPoint,prevposition);
